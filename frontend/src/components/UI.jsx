...
export const UI = ({ hidden, meta_ui }) => {
  ...
  async function fetchCredits() {
    const resp = await fetch(`${import.meta.env.VITE_BACKENDADDR}/credits`, {
      headers: {
  ...
  useEffect(() => {
    async function load_data() {
      const resp = await fetch(
        `${import.meta.env.VITE_BACKENDADDR}/history/${chatId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const speechRecon = () => {
    try {
      ...
      window.rec.addEventListener("error", (e) =>
        console.log("Recognition error:", e.error, e),
      );
      window.rec.addEventListener("nomatch", () =>
    ...
    <>
      <ErrorBoundary FallbackComponent={ErrorFallBack}>
            ...
            >
              ...
              <div className="flex space-y-4 flex-col overflow-y-scroll">
                {chatIds.map((value) => (
                  <div
                    key={value.id}
                    className="bg-slate-700 text-white p-2 text-center rounded-md flex justify-between items-center"
                  >
                    <Link to={`/chat/${value.id}`}> {value.gist}</Link>
                    <MdDeleteOutline
                      className="text-3xl bg-red-700 rounded-md"
                      onClick={async () => await handleDeleteHistory(value.id)}
                    />
                  </div>
                ))}
              </div>
        ...
        <div className="fixed top-0 left-0 right-0 bottom-0 z-10 flex justify-between p-4 flex-col pointer-events-none">
          ...
          <div className="pointer-events-auto bg-black/70 flex justify-between text-white p-3 rounded-xl shadow-lg  transition-all">
            <div className="flex items-center justify-center space-x-5">
              <button onClick={() => setSidebarOpen(!sidebarOpen)}>
                <img
                  src={localStorage.getItem("pic")}
                  className="rounded-full size-12 items-center justify-center border-2 border-white"
                />
              </button>
              <button
            ...
            <button className="bg-black/70 border-2 border-blue-700 hover:bg-blue-500/70 text-white p-3 rounded-xl transition-all">
          ...
          {toggleContextHistory && (
            <div
              ref={chatEndRef}
              className="bg-gray-900 opacity-90 border-blue-400 border-2 shadow-[0_0_5px_#60A5FA,0_0_5px_#60A5FA,0_0_10px_#3B82F6,0_0_30px_#2563EB]  h-[60%] w-[30%] fixed left-16 bottom-[20%] text-center rounded-xl overflow-scroll pointer-events-auto"
            >
          ...
          <div>
            <div className="flex items-center justify-center">
              <span
                className="text-white p-2 px-4 rounded-full text-center m-2 bg-black/70 text-xl mb-8 shadow-lg"
                id="caption"
              >
                {speechCaption}
              </span>
